...
class WorkflowE2ETest {
    ...
    @Test
    void t07_signupRoleAndAdminRoleAssignmentAffectDashboard() throws Exception {
        JsonNode registered = postJson("/api/auth/register", """
            {"name":"New Manager","email":"new.manager@example.com","password":"secret123",
             "department":"Engineering","role":"manager"}
            """, 201);
        assertThat(registered.get("role").asText()).isEqualTo("manager");

        JsonNode updated = call(put("/api/users/" + EMPLOYEE)
                .contentType(MediaType.APPLICATION_JSON)
                .content("{\"role\":\"manager\"}"), 200);
        assertThat(updated.get("role").asText()).isEqualTo("manager");

        JsonNode dashboard = call(get("/api/dashboard/user").param("userId", EMPLOYEE), 200);
        assertThat(dashboard.has("pendingRequests") || dashboard.has("recentRequests")).isTrue();
    }
}
