...
// ── Payments (finance) ───────────────────────────────────────
/**
 * Payment lifecycle. A payment is raised automatically when goods are
 * delivered against a purchase order, then finance runs the three-way
 * match (PO + GRN + invoice), releases the payment and finally confirms
 * the bank settlement.
 */
export const PAYMENT_FLOW = {
  pending: ['verified', 'on_hold'],
  on_hold: ['pending', 'verified'],
  verified: ['processing', 'on_hold'],
  processing: ['paid', 'failed'],
  failed: ['processing', 'on_hold'],
  paid: [],
};

export const PAYMENT_STAGE_ORDER = ['pending', 'verified', 'processing', 'paid'];

export const PAYMENT_ACTION_LABELS = {
  verified: 'Verify Invoice',
  processing: 'Release Payment',
  paid: 'Confirm Settlement',
  on_hold: 'Put On Hold',
  failed: 'Mark Failed',
  pending: 'Reopen',
};

export const PAYMENT_METHODS = ['NEFT', 'RTGS', 'IMPS', 'UPI', 'Cheque'];

const FINANCE_ROLES = ['finance_officer', 'admin'];

export const canProcessPayments = (user) => FINANCE_ROLES.includes(user?.role);

const assertFinance = (user, action) => {
  if (!canProcessPayments(user)) {
    throw new Error(`Only the finance team can ${action}.`);
  }
};

export const getPayments = ({ status, search } = {}) =>
  payments
    .filter((p) => !status || status === 'all' || p.status === status)
    .filter((p) => {
      if (!search) return true;
      const q = search.toLowerCase();
      return [p.id, p.poNumber, p.supplierName, p.referenceNumber, p.transactionId]
        .some((v) => String(v || '').toLowerCase().includes(q));
    })
    .sort((a, b) => String(b.id).localeCompare(String(a.id)));

export const getPayment = (id) => payments.find((p) => p.id === id);

export const canTransitionPayment = (from, to) => (PAYMENT_FLOW[from] || []).includes(to);

export const availablePaymentActions = (payment, user) => {
  if (!payment || !canProcessPayments(user)) return [];
  return (PAYMENT_FLOW[payment.status] || []).map((status) => ({
    status,
    label: PAYMENT_ACTION_LABELS[status] || status,
  }));
};

export const paymentProgress = (payment) => {
  const index = PAYMENT_STAGE_ORDER.indexOf(payment?.status);
  return PAYMENT_STAGE_ORDER.map((stage, i) => ({
    stage,
    done: index >= 0 && i <= index,
    current: stage === payment?.status,
  }));
};

const addPaymentHistory = (payment, user, status, remarks) => {
  payment.history = [
    ...(payment.history || []),
    { status, by: user?.name || 'System', role: user?.role || 'system', at: stamp(), remarks: remarks || '' },
  ];
};

const supplierIdFor = (name) => suppliers.find((s) => s.companyName === name)?.id;

/** Raise a payment for a delivered purchase order (idempotent). */
export const ensurePaymentForPo = (po, user) => {
  if (!po) return null;
  const existing = payments.find((p) => p.poNumber === po.id || p.poId === po.id);
  if (existing) return existing;
  const payment = {
    id: nextId('PAY-2024-', payments),
    poId: po.id,
    poNumber: po.id,
    requestId: po.requestId || null,
    supplierId: po.supplierId || supplierIdFor(po.supplierName),
    supplierName: po.supplierName,
    invoiceNumber: `INV-${String(po.id).replace(/[^0-9]/g, '') || payments.length + 1}`,
    ...
    remarks: '',
    createdAt: stamp(),
    history: [{ status: 'pending', by: user?.name || 'System', role: user?.role || 'system', at: stamp(), remarks: 'Payment raised on delivery.' }],
  };
  ...
  recordAudit({
    ...
    entityId: payment.id,
    updatedValue: 'pending',
    remarks: `Payment raised for ${po.id} (${po.supplierName}).`,
  });
  pushToRoles(['finance_officer'], {
    type: 'invoice_pending',
    title: 'Invoice Pending Verification',
    message: `${payment.id} for ${po.id} (₹${Math.round(payment.amount).toLocaleString('en-IN')}) is awaiting verification.`,
...
const REQUIRED_CHECKS = ['poVerified', 'grnVerified', 'invoiceVerified', 'taxVerified', 'amountVerified'];

/** Three-way match — every check must pass before the invoice is approved. */
export const verifyInvoice = (paymentId, { checks = {}, remarks = '', user }) => {
  assertFinance(user, 'verify invoices');
  const payment = getPayment(paymentId);
  if (!payment) throw new Error('Payment not found');
  if (!canTransitionPayment(payment.status, 'verified')) {
    throw new Error(`${payment.id} cannot be verified from ${payment.status}.`);
  }
  const missing = REQUIRED_CHECKS.filter((c) => !checks[c]);
  if (missing.length) {
    recordAudit({
      user,
      action: 'VERIFY_INVOICE_REJECTED',
      entity: 'Payment',
      entityId: payment.id,
      previousValue: payment.status,
      remarks: `Blocked: incomplete three-way match (${missing.join(', ')}).`,
    });
    throw new Error('All five verification checks must pass before the invoice can be approved.');
  }
  const previous = payment.status;
  payment.status = 'verified';
  payment.checks = { ...checks };
  payment.verifiedBy = user?.id || null;
  payment.verifiedByName = user?.name || null;
  payment.verifiedAt = stamp();
  payment.remarks = remarks || payment.remarks;
  addPaymentHistory(payment, user, 'verified', remarks || 'Three-way match passed.');
  recordAudit({
    user,
    ...
    remarks: remarks || `Invoice ${payment.invoiceNumber || ''} verified against ${payment.poNumber}.`.trim(),
  });
  emitChange();
  return payment;
};

/** Release the payment to the supplier's bank (pending settlement). */
export const releasePayment = (paymentId, { paymentMethod, referenceNumber, remarks = '', user }) => {
  assertFinance(user, 'release payments');
  const payment = getPayment(paymentId);
  if (!payment) throw new Error('Payment not found');
  if (payment.status === 'pending') {
    throw new Error(`${payment.id} must pass invoice verification before it can be released.`);
  }
  if (!canTransitionPayment(payment.status, 'processing')) {
    throw new Error(`${payment.id} cannot be released from ${payment.status}.`);
  }
  ...
  payment.referenceNumber =
    referenceNumber ||
    `${paymentMethod.toUpperCase()}-${stamp().slice(0, 10).replace(/-/g, '')}-${payment.id.slice(-3)}`;
  payment.releasedBy = user?.name || null;
  payment.releasedAt = stamp();
  addPaymentHistory(payment, user, 'processing', remarks || `Released via ${paymentMethod}.`);
  recordAudit({
    user,
    ...
    entityId: payment.id,
    previousValue: previous,
    updatedValue: 'processing',
    remarks: remarks || `₹${Math.round(payment.amount).toLocaleString('en-IN')} released via ${paymentMethod} (${payment.referenceNumber}).`,
  });
  if (payment.supplierId) {
    pushToSupplier(payment.supplierId, {
      type: 'payment_initiated',
      title: 'Payment Released',
      message: `Payment of ₹${Math.round(payment.amount).toLocaleString('en-IN')} for ${payment.poNumber} is in process (${payment.referenceNumber}).`,
      link: '/supplier-portal',
    });
  }
  emitChange();
...
/** Confirm the bank settlement — the payment is now complete. */
export const confirmPayment = (paymentId, { transactionId, remarks = '', user }) => {
  assertFinance(user, 'confirm payments');
  const payment = getPayment(paymentId);
  if (!payment) throw new Error('Payment not found');
  if (!canTransitionPayment(payment.status, 'paid')) {
    throw new Error(`${payment.id} cannot be settled from ${payment.status}.`);
  }
  const previous = payment.status;
  payment.status = 'paid';
  payment.transactionId = transactionId || `TXN-${Date.now()}`;
  payment.paidDate = stamp().split('T')[0];
  addPaymentHistory(payment, user, 'paid', remarks || 'Settlement confirmed.');
  recordAudit({
    user,
    ...
    previousValue: previous,
    updatedValue: 'paid',
    remarks: remarks || `Settled — ${payment.transactionId}.`,
  });
  if (payment.supplierId) {
    pushToSupplier(payment.supplierId, {
      type: 'payment_completed',
      title: 'Payment Completed',
      message: `₹${Math.round(payment.amount).toLocaleString('en-IN')} for ${payment.poNumber} has been settled (${payment.transactionId}).`,
      link: '/supplier-portal',
    });
  }
  pushToRoles(['procurement_officer'], {
    type: 'payment_completed',
    title: 'Supplier Paid',
    message: `${payment.poNumber} — ${payment.supplierName} has been paid.`,
    link: '/finance/payments',
  });
  emitChange();
  return payment;
...
/** Hold / fail / reopen a payment. */
export const updatePaymentStatus = (paymentId, nextStatus, { remarks = '', user } = {}) => {
  assertFinance(user, 'process payments');
  const payment = getPayment(paymentId);
  if (!payment) throw new Error('Payment not found');
  if (!canTransitionPayment(payment.status, nextStatus)) {
    throw new Error(`${payment.id} cannot move from ${payment.status} to ${nextStatus}.`);
  }
  const previous = payment.status;
  payment.status = nextStatus;
  addPaymentHistory(payment, user, nextStatus, remarks);
  recordAudit({
    user,
    action: `PAYMENT_${nextStatus.toUpperCase()}`,
    entity: 'Payment',
    entityId: payment.id,
    previousValue: previous,
    updatedValue: nextStatus,
    remarks: remarks || `Payment moved to ${nextStatus.replace(/_/g, ' ')}.`,
  });
  emitChange();
  return payment;
};
...
export const getPaymentSummary = () => {
  const total = (list) => list.reduce((sum, p) => sum + (p.amount || 0), 0);
  ...
  return {
    ...
    onHold: by('on_hold'),
    failed: by('failed'),
    payableAmount: total(payments.filter((p) => ['pending', 'verified', 'processing'].includes(p.status))),
    paidAmount: total(by('paid')),
  };
};
